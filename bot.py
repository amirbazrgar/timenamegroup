...
async def update_group_name():
    try:
        ...
        fancy_time = convert_time(now.strftime("%H:%M"), font_name)
        new_name = f"{group_name} {fancy_time}"
        await bot.edit_group_info(group_guid, title=new_name)
        print(f"[✅] اسم جدید گروه تنظیم شد: {new_name} با فونت {font_name}")
...
async def main():
    ...
    scheduler.add_job(update_group_name, "cron", second=0)
    scheduler.start()
